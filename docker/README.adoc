...
If you are new to Docker or Kill Bill, see our https://github.com/killbill/killbill-cloud/tree/master/docker/compose[Docker Compose recipes] instead.
...
To start Kill Bill 0.18.x: (`x` being the latest 0.18 release out, see https://hub.docker.com/r/killbill/killbill/tags/[Docker Hub]):

....
docker run -ti -p 8080:8080 killbill/killbill:0.18.x
....

Use `docker-machine env <name>` or the environment variable `$DOCKER_HOST` to get the IP address of the container. You can verify the Kill Bill status by checking the healthcheck:

....
curl -v http://<IP_ADDRESS>:8080/1.0/healthcheck
....
...
There is a https://issues.sonatype.org/browse/OSSRH-13936[bug in Sonatype where the sha1 is sometimes wrong]. In order to disable sha1 verification, you can start your container using: `KPM_PROPS="--verify-sha1=false"`.

Configuration of the images is driven by the https://github.com/killbill/killbill-cloud/blob/master/docker/templates/killbill/latest/kpm.yml.erb[kpm.yml.erb] KPM configuration file. Advanced users may need to extend it beyond the plugin properties exposed. To do so, you can create an overlay file as `$KILLBILL_CONFIG/kpm.yml.erb.overlay`: both files will be merged at startup, with the overlay having precedence. Take a look at our https://github.com/killbill/killbill-cloud/blob/master/docker/templates/killbill/testing/Dockerfile[testing] image for an example.
...
* Kill Bill http://docs.killbill.io/0.18/ddl.sql[DDL]
+
`curl -s http://docs.killbill.io/0.18/ddl.sql | docker exec -i db mysql -h localhost -uroot -proot -D killbill`
* Analytics https://github.com/killbill/killbill-analytics-plugin/blob/master/src/main/resources/org/killbill/billing/plugin/analytics/ddl.sql[DDL]
+
...
* Stripe https://github.com/killbill/killbill-stripe-plugin/blob/master/db/ddl.sql[DDL]
+
...
1.  Start the killbill container with the two plugins `analytics` and `stripe`:
...
* Add the https://raw.githubusercontent.com/killbill/killbill-admin-ui/master/db/ddl.sql[DDL] for KAUI
...
1.  Import your SSL certificate (see https://tomcat.apache.org/tomcat-7.0-doc/ssl-howto.html[docs]). For testing, you can just create a self-signed certificate:
...
1.  Enable the `RemoteIpValve` (make sure to configure correctly at least the `internalProxies` and `trustedProxies` attributes depending on your environment, see the https://tomcat.apache.org/tomcat-7.0-doc/config/valve.html#Proxies_Support[docs]):
