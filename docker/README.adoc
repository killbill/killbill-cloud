...
If you are new to Docker or Kill Bill, see our https://github.com/killbill/killbill-cloud/tree/master/docker/compose[Docker Compose recipes] first.
...
+
....
docker run -ti -p 8080:8080 -p 8443:8443 killbill/killbill:0.18.x
....

2.  Import your SSL certificate (see https://tomcat.apache.org/tomcat-8.5-doc/ssl-howto.html[docs]). For testing, you can just create a self-signed certificate:
+
....
sudo apt-get update
...
....
# In /var/lib/tomcat/conf/server.xml
