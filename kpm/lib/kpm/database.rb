  ...
  class Database
    ...
    def execute_insert_statement(table_name, query, qty_to_insert, _table_data, record_id = nil)
      ...
      if response.include? 'ERROR'
        ...
        if response.include? 'Table' and response.include? 'doesn\'t exist'
          @logger.info "Skipping unknown table #{table_name}...."
        else
          raise Interrupt, "Importing table #{table_name}...... \e[91;1m#{response}\e[0m" end
      end
